...
const Navbar = () => {
  ...
  const [isCartOpen, setIsCartOpen] = useState(false);
  const navigate = useNavigate();
  const { cart, removeItem, courseTitles } = useCart(); // Use the cart state, removeItem function, and courseTitles from the context
  const [avatarUrl, setAvatarUrl] = useState('');
              ...
              {cart.length > 0 ? (
                cart.map((item) => (
                  <div key={item.id} className="flex items-center justify-between mb-4 border-b pb-2">
                    <img src={item.image} alt={item.name} className="w-10 h-10 object-cover" />
                    <div className="flex-1 ml-4">
                      <p className="text-sm font-medium">{item.title}</p> {/* Display course title */}
                      <p className="text-sm text-gray-500">{item.description}</p>
                      <p className="text-sm text-gray-500">
                        {item.quantity} × ${item.price}
                      </p>
