...
const Navbar = () => {
  const demoData = [
    ...
    [
      ...
      { title: 'Demo Start →', image: coursebuy, link: '/coursebuy' },
      { title: 'Demo Start →', image: reactpage, link: '/react' },
      { title: 'Demo Start →', image: loginpage, link: '/login' },
    ],
    [
      { title: 'Demo Start →', image: signuppage, link: '/signup' },
        ...
        <li key={menu.name} className="border-b">
          ...
          <div className="flex items-center gap-4">
              ...
              <span className=" flex items-center gap-1 text-gray-800">
                ...
                <button className="relative px-3 py-1 border border-purple-500 text-purple-500 rounded-md hover:bg-purple-500 hover:text-white">
                  Purchase Now
                </button>
            </div>  
       {/* Overlay */}
       {isOpen && (
        <div className="fixed inset-0 bg-red opacity-50 z-[999] md:hidden" onClick={() => setIsOpen(false)}>hi</div>
      )}
       </div>
    </div>
  );
