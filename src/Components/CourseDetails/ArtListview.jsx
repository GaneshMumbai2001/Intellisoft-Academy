      ...
      >
        {courses.map((course, index) => (
          <motion.div
            ...
            viewport={{ once: true }}
          >
              <Link to={`/pages/${course.id}`} className="bg-white shadow-lg rounded-xl p-5 flex flex-wrap gap-6 transition-transform hover:scale-105 m-8">
            <img className="w-40 h-40 rounded-lg md:w-44 sm:w-48" src={course.imageLink} alt={course.courseName} />
...
//       <motion.div
//         className="relative w-full lg:max-w-5xl flex flex-col gap-6 md:translate-y-[-90px] sm:translate-y-[100px]"
//         initial={{ opacity: 0, y: 50 }}
//         whileInView={{ opacity: 1, y: 0 }}
//         transition={{ duration: 0.5 }}
//         viewport={{ once: true }}
...
//           <motion.div
//             key={index}
//             className="bg-white shadow-lg rounded-xl p-5 flex items-center gap-6 transition-transform hover:scale-105 m-4"
...
//           >
//             {/* Course Image */}
//             <Link to={`/pages/${course.id}`} className="text-lg font-semibold text-gray-900">
//             <img className="w-40 h-40 rounded-lg md:w-44 sm:w-48" src={course.imageLink} alt={course.courseName} />
//             </Link>
//             {/* Course Details */}
...
//               {/* Instructor Info */}
//               <div className="flex items-center mt-3">
//                 <img className="h-8 w-8 rounded-full mr-2" src={course.instructor.imageLink} alt={course.instructor.name} />
//                 <span className="text-sm text-gray-700">By {course.instructor.name} in {course.instructor.field}</span>
//               </div>
...
//                   <s className="text-gray-400 text-sm">${course.price}</s> ${course.curPrice}
//                 </div>
//                 <button className="bg-gray-100 text-black px-4 py-2 shadow rounded-lg text-sm hover:bg-gray-200 cursor-pointer">
//                   Add to cart
//                 </button>
//               </div>
//             </div>
