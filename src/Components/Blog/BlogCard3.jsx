    ...
    <div className="p-5 max-w-5xl mx-auto ">
      ...
      <div className="py-10">
        <div className="bg-white shadow-lg flex flex-wrap items-center rounded-lg p-5 max-w-3xl mx-auto">
          <img
            src={images[2]} // Example: Display image from third index (could be dynamic)
            alt={titles[2]} // Example: Display title from third index
            className="w-full h-60 object-cover rounded"
          />
          <h2 className="text-xl font-bold mt-2">{titles[2]}</h2>
          <p className="text-gray-600">{descriptions[2]}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 mt-5">
        {images.map((image, index) => (
          <div key={index} className="bg-white p-4 shadow-md  flex flex-wrap  rounded-lg  items-center space-x-4">
            <img
